...
const Border = ({ width, height, isLandscape }) => (
  <svg
  ...
  >
    {(isLandscape ? getLandscapePaths : getPortraitPaths)(width, height).map(
      (path, index) => {
        return <path d={path} key={index} fill="none" stroke="currentColor" />
      }
    )}
  </svg>
)
...
export const EventCard = ({
  ...
  return (
    ...
    >
      <EventLink
        href={`/events/${uid}`}
        className="block pt-4 pb-6 lg:p-2 relative"
      >
        <Border width={width} height={height} isLandscape={isLandscape} />
